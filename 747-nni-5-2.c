...
void print(Tree* Root)
{
	...
	int TmpForMass =0;
	int TmpForMass2 =0;
	TempNode * TempStruct = Root -> root;
	while(TmpForMass2 <Root ->count)
	{
		while (TempStruct!= NULL)
		{
			TmpForMass2 = TmpForMass2 + 1;
			if(TempStruct -> right != NULL)
			{TmpForMass = TmpForMass + 1 ;
				mass[TmpForMass] = TempStruct->right;
			}
			mass2[TmpForMass2] = TempStruct;
			TempStruct = TempStruct->left;
			
		}
		TempStruct = mass[TmpForMass];
		TmpForMass = TmpForMass - 1 ;
	}
	for (int i = 1 ;i <= TmpForMass2;i++)
	{
	    mass2[i] = mass2[i] -> value;
		printf("%d ",mass2[i]);
}
}
void printTree(Tree *tree) {
